...
module.exports = APIRequest.inherit({
    ...
    route: config.apiserver.prefix+'/rules/bb/raw',
	
	gen_process: coroutine(function*(data, request, g) {

        var answer = { }

		answer.raw = yield bridge_mysqlpool.query('SELECT * FROM bb_rule where login_or_group = ? order by id', [ '' + data.login ], g.resume)
