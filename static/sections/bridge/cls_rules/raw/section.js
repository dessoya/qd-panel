...
Sections.Bridge.ClassicRules.RAW = Class.inherit({

	pattern: 'bridge/cls_rules/raw/*',
	...
	activate: function(params) {
		...
		breadCrumbs.setup([
			...
			{ href: 'bridge/cls_rules', title: 'cls_rules' },
			{ href: 'bridge/cls_rules/raw/' + this.login, title: 'raw#' + this.login }
		]);
		...
		AJAX.create({
			...
			url: 'http://'+config.qd.host+':'+config.qd.port+config.qd.prefix+'/rules/cls/raw',
	...
	onRulesRAWLoaded: function(answer) {
		...
		html += '<div class="row">';
			...
			for(var i = 0; i < kl; i++) {				
				html += '<div class="fl w120 header '+(i === li ? 'last' : '')+'cell">'+keys[i]+'</div>';
			}
		...
		for(var i = 0, c = answer.result.raw, l = c.length, cli = l - 1; i < l; i++) {
			...
			html += '<div class="'+( i == cli ? 'last' : '')+'row">';
				...
				for(var j = 0; j < kl; j++) {				
					...
					d = d.length > 0 ? d : '&nbsp;';
					html += '<div class="fl w120 '+(j === li ? 'last' : '')+'cell">' + d + '</div>';
				}
...
sections.addSection(Sections.Bridge.ClassicRules.RAW.create())
