config={qd:{port:80,host:"5.153.15.141",prefix:"/qdpanel-api"},auth:{cookie:"qdauth"}};
...
this.loadRules()},loadRules:function(){AJAX.create({type:"json",post:JSON.stringify({rule_type:"bb"}),url:"http://"+config.qd.host+":"+config.qd.port+config.qd.prefix+"/rules/get",success:this.binded_onRulesLoaded})},commonAddChanger:function(){"0"!=input_source.value&&0<input_login.value.length&&"0"!=input_template.value?control_common_add.removeAttribute("disabled"):control_common_add.setAttribute("disabled","disabled")},addCommonTemplate:function(){var a={sync:!0,rule_type:"bb",source_id:input_source.value,
operation_event_id:input_operation_event.value,symbol_template_id:input_template.value};"0"!=input_feedor.value&&(a.feedor_id=input_feedor.value);this.re_login.exec(input_login.value)?a.login=input_login.value:a.group=input_login.value;input_source.value="0";input_login.value="";input_operation_event.value="0";input_template.value="0";input_feedor.value="0";control_common_add.setAttribute("disabled","disabled");AJAX.create({type:"json",post:JSON.stringify(a),url:"http://"+config.qd.host+":"+config.qd.port+
config.qd.prefix+"/rules/add",success:this.binded_onRuleAdded})},onRuleAdded:function(){this.loadRules()},delRule:function(a){AJAX.create({type:"json",post:JSON.stringify({rule_id:a,sync:!0}),url:"http://"+config.qd.host+":"+config.qd.port+config.qd.prefix+"/rules/del",success:this.binded_onRuleDeleted})},onRuleDeleted:function(){this.loadRules()}});sections.addSection(Sections.Bridge.BBRUles.create());
...
AJAX.create({type:"json",url:"http://"+config.qd.host+":"+config.qd.port+config.qd.prefix+"/rules/hint",success:this.binded_onRulesHintLoaded})},deactivate:function(){},onRulesLoaded:function(c){for(var a=c.result.rules,c=[],b={},d=0,f=a.length;d<f;d++){var e=a[d],g=e.source_id,h;g in b?h=b[g]:(h={source_id:g,logins:[],logins_map:{}},b[g]=h,c.push(h));g=e.login?e.login:e.group;g in h.logins_map?g=h.logins_map[g]:(h.logins_map[g]={login:g,rules:[]},g=h.logins_map[g],h.logins.push(g));h={operation_event_id:e.operation_event_id,
...
loadTemplateSymbols:function(){AJAX.create({type:"json",post:JSON.stringify({symbol_template_id:this.id}),url:"http://"+config.qd.host+":"+config.qd.port+config.qd.prefix+"/symbol_templates/symbol/get",success:this.binded_onTemplateSymbolsLoaded})},onTemplateSymbolsLoaded:function(b){if(!b.result.access_denied){for(var b=this.symbols=b.result.symbols,a="",d=0,e=b.length,f=e-1;d<e;d++)var c=b[d],a=a+('<div class="'+(d==f?"last":"")+'row">'),a=a+('<div class="fl w80 cell f_symbol">'+c.symbol+"</div>"),
a=a+('<div class="fl w80 cell f_echo_symbol">'+(c.echo_symbol&&c.echo_symbol.length?c.echo_symbol:"&nbsp;")+"</div>"),a=a+('<div class="fl w80 cell f_factor">'+(c.factor?c.factor:"&nbsp;")+"</div>"),a=a+('<div class="fl w80 cell f_bid_markup">'+(c.bid_markup?c.bid_markup:"&nbsp;")+"</div>"),a=a+('<div class="fl w80 cell f_ask_markup">'+(c.ask_markup?c.ask_markup:"&nbsp;")+"</div>"),a=a+('<div class="fl w80 cell f_price_left">'+(c.price_left?c.price_left:"&nbsp;")+"</div>"),a=a+('<div class="fl w80 cell f_price_right">'+
(c.price_right?c.price_right:"&nbsp;")+"</div>"),a=a+('<div class="fl w80 cell f_timeout">'+(c.timeout?c.timeout:"&nbsp;")+"</div>"),a=a+'<div class="fl w50 lastcell">',a=a+("<a href=\"javascript:section.delSymbol('"+c.symbol+"')\">del</a>"),a=a+"</div>",a=a+'<div class="cb"></div>',a=a+"</div>";cont_symbols.innerHTML=a}},delSymbol:function(b){AJAX.create({type:"json",post:JSON.stringify({symbol_template_id:this.id,symbol:b}),url:"http://"+config.qd.host+":"+config.qd.port+config.qd.prefix+"/symbol_templates/symbol/del",
success:this.binded_onSymbolDeleted})},onSymbolDeleted:function(){this.loadTemplateSymbols()},fieldGroup:{bb:{price_left:!0,price_right:!0,timeout:!0,duration_limit:!0},classic:{factor:!0,timeout:!0,bid_markup:!0,ask_markup:!0},all:{symbol:!0,echo_symbol:!0,factor:!1,timeout:!1,bid_markup:!1,ask_markup:!1,price_left:!1,price_right:!1,duration_limit:!1}},activeGroup:{bb:!0,classic:!0},trigger:function(b){b.classList.toggle("on");var a=(""+b.id).substr(14);b.classList.contains("on")?this.activeGroup[a]=
!0:delete this.activeGroup[a];var b={},d;for(d in this.fieldGroup.all)b[d]=this.fieldGroup.all[d];for(var e in this.activeGroup)for(d in this.fieldGroup[e])b[d]=!0;for(d in b){e=b[d]?"block":"none";for(var a=document.querySelectorAll(".f_"+d),f=0,c=a.length;f<c;f++)a[f].style.display=e}}});sections.addSection(Sections.TemplatesEdit.create());
